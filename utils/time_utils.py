...
class DeformNetwork(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, output_ch=59, t_multires=6, multires=10,
        ...
        for layer in self.linear:  # self.linear 作用于位置和时间的输入
        ...
        nn.init.zeros_(self.gaussian_warp.bias)  # 输出dxyz
...
def farthest_point_sample(xyz, npoint):
    ...
    """
    # 最远点下采样,每次采样一个点后使用距离当前点最远的的点作为下一个采样点。
    device = xyz.device
    B, N, C = xyz.shape  # 1, N, 3
    centroids = torch.zeros(B, npoint, dtype=torch.long).to(device)
    distance = torch.ones(B, N).to(device) * 1e10
    farthest = torch.randint(0, N, (B,), dtype=torch.long).to(device)  # 随机挑选一个点作为最远点
    batch_indices = torch.arange(B, dtype=torch.long).to(device)
    for i in range(npoint):
        centroids[:, i] = farthest
        centroid = xyz[batch_indices, farthest, :].view(B, 1, C)  # 挑选点位置
        dist = torch.sum((xyz - centroid) ** 2, -1)  # 挑选点与所有点的距离
        mask = dist < distance
        distance[mask] = dist[mask]
        farthest = torch.max(distance, -1)[1]  # 将最远点作为下一个挑选点
    return centroids
...
class ControlNodeWarp(nn.Module):
    def __init__(self, is_blender, init_pcl=None, node_num=512, K=5, use_hash=False, hash_time=False, enable_densify_prune=False, pred_opacity=False, pred_color=False, with_arap_loss=False, with_node_weight=True, local_frame=False, d_rot_as_res=True, skinning=False, hyper_dim=2, progressive_brand_time=False, max_d_scale=-1, is_scene_static=False, **kwargs):
        ...
        self.nodes = nn.Parameter(torch.randn(node_num, 3+self.hyper_dim))  # [node_num, 3(xyz点位置) + 8(？+旋转四元数+平移向量)]
        if not self.skinning:
            self._node_radius = nn.Parameter(torch.randn(node_num))  #_node_radius作用：densify和计算权重cal_nn_weight时使用到
    ...
    def init(self, opt, init_pcl, hyper_pcl=None, keep_all=False, force_init=False, as_gs_force_with_motion_mask=False, force_gs_keep_all=False, reset_bbox=True, **kwargs):
        ...
        if keep_all or self.node_num > init_pcl.shape[0]:  # 设置控制点数量大于输入点云数量情况:
            self.nodes = nn.Parameter(torch.cat([init_pcl.float(), 1e-2 * torch.ones([init_pcl.shape[0], self.hyper_dim]).float().cuda()], dim=-1))  # 使用所有高斯点位置初始化控制点
            init_nodes_idx = None
            ...
            self.nodes.data = nn.Parameter(torch.cat([init_pcl[init_nodes_idx].float(), 1e-2 * torch.ones([self.node_num, self.hyper_dim]).float().cuda()], dim=-1))  # [node_num, 3(xyz点位置) + 8(半径o+旋转四元数+平移向量)] 将3(xyz点位置)用init_pcl赋值
        ...
        if self.skinning:
            ...
            if keep_all or self.node_num > init_pcl.shape[0]:
                self._node_radius = nn.Parameter(torch.log(.1 * scene_range + 1e-7) * torch.ones([self.node_num]).float().to(scene_range.device))  # 初始化控制点的半径
    ...
    def extend_node(self, init_pcl, keep_all=False, force_init=False, reset_bbox=True, sample_number=250, **kwargs):
        if keep_all or self.node_num > init_pcl.shape[0]:  # 设置控制点数量大于输入点云数量情况:
            new_nodes = nn.Parameter(torch.cat([init_pcl.float(), 1e-2 * torch.ones([init_pcl.shape[0], self.hyper_dim]).float().cuda()], dim=-1))  # 使用所有高斯点位置初始化控制点
            init_nodes_idx = None
            ...
            new_nodes = nn.Parameter(torch.cat([init_pcl[init_nodes_idx].float(), 1e-2 * torch.ones([sample_number, self.hyper_dim]).float().cuda()], dim=-1))  # [node_num, 3(xyz点位置) + 8(半径o+旋转四元数+平移向量)] 将3(xyz点位置)用init_pcl赋值
        scene_range = init_pcl.max() - init_pcl.min()
        if self.skinning:
            if 'feature' in kwargs:
                gs_weights = kwargs['feature']
            ...
            if keep_all or self.node_num > init_pcl.shape[0]:
                new_node_radius = nn.Parameter(torch.log(.1 * scene_range + 1e-7) * torch.ones([new_nodes.shape[0]]).float().to(scene_range.device))  # 初始化控制点的半径
    ...
    def cal_nn_weight(self, x:torch.Tensor, K=None, feature=None, nodes=None, gs_kernel=True, temperature=1.):
        ...
        if self.skinning:
            ...
            if self.cached_nn_weight and self.nn_weight is not None:
                ...
                if gs_kernel:
                    nn_radius = self.node_radius[nn_idxs]  # N, K  # 函数返回self._node_radius
    ...
    def query_network(self, x, t, **kwargs):
        values = self.network(x=x, t=t, **kwargs)  # 410行的forward()
        return values
    ...
    def elastic_loss(self, t=None, delta_t=0.005, K=2, t_samp_num=8, cur_time=None):  # APAR Loss in paper
        ...
        if cur_time is not None:
            t = torch.clamp(t, 0+0.5*delta_t, cur_time-0.5*delta_t)
        # 生成t_samp_num个采样时间点，范围在[t - 0.5 * delta_t, t + 0.5 * delta_t]之间
        t_samp = torch.rand(t_samp_num).cuda() * delta_t + t - .5 * delta_t  # 生成[-delta_t + t, delta_t + t]之间的8个数
        #if cur_time is not None:
        #    t_samp = torch.clamp(t_samp, 0, cur_time)
    ...
    def forward(self, x, t, feature, motion_mask, iteration=0, is_training=True, node_trans_bias=None, node_scaling_bias=None, animation_d_values=None, **kwargs):
        # 通过MLP预测控制点的dxyz, dscale, drot, 使用cal_nn_weight[N, 3]计算控制点对3个相近高斯的权重
        if t.dim() == 0:
    ...
    def densify(self, max_grad, optimizer, x:torch.Tensor, x_grad: torch.Tensor, feature=None, K=None, use_gaussians_grad=False, force_dp=False):
        ...
        if pruned_pts_mask.sum() > 0:
            ...
            param_idx = np.arange(len(param_list))
            for group in optimizer.param_groups:
                if group["name"] != 'nodes':  #  {'params': [self.nodes, self._node_radius], 'name': 'nodes'} prune self.nodes和self._node_radius参数
                    continue
                for i in param_idx:
                    stored_state = optimizer.state.get(group['params'][i], None)
                    if stored_state is not None:
            ...
            for key in optimizable_tensors:
                setattr(self, key, optimizable_tensors[key])  # 将prune结果保存到self.nodes和self._node_radius中
        else:
            pruned_pts_mask = ~pruned_pts_mask
        ...
        #self.gs._xyz.data = self.nodes[..., :3]  # self.node定义的高斯self.gs有什么作用,只在预训练中有使用到？？
