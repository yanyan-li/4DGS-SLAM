...
class ParamGroup:
    def __init__(self, parser: ArgumentParser, name : str, fill_none = False):
        ...
        for key, value in vars(self).items():  # vars 返回对象的 __dict__ 属性
            shorthand = False
            if key.startswith("_"):  # 删除键前面的_
                shorthand = True
                key = key[1:]
            t = type(value)  # 获取值的数据类型
            value = value if not fill_none else None  # 是否要把值赋值为None
            if shorthand:  # 可以输入缩写
                if t == bool:
...
class ModelParams(ParamGroup): 
    ...
    def extract(self, args):  # 获得__init__函数中设置的dict对象{sh_degree:3, source_path:" ", ...}
        g = super().extract(args)
        g.source_path = os.path.abspath(g.source_path)  # 将g.source_path设置为绝对路径
        return g
...
class ModelHiddenParams(ParamGroup):
    def __init__(self, parser):
        ...
        self.no_ds=False # cancel the deformation of Gaussians' scaling  # dynamic-SLAM可能不需要？？
